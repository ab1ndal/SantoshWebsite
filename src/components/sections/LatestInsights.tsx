...
const posts = [
  ...
  },
  {
    ...
    tag: "Comparison",
    tagColor: "text-ink-200 border-ink-400/30",
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <h2
              className="text-[clamp(2rem,4vw,2.8rem)] text-white"
          ...
          <Link
            href="/insights"
            className="text-green-400 font-semibold hover:text-green-300 transition-colors whitespace-nowrap"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {posts.map((post) => (
            <Link
              ...
              className="group flex flex-col p-6 rounded-xl border border-ink-600/40 bg-ink-800/50 hover:border-green-500/30 hover:bg-ink-800 transition-all duration-300"
            >
              ...
              <h3
                className="text-base font-bold text-white mb-3 leading-snug group-hover:text-green-300 transition-colors"
                style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}
              >
                {post.title}
              </h3>
              <p
                className="text-sm text-ink-200 leading-relaxed flex-1 mb-5"
                style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 300 }}
              >
                {post.excerpt}
              </p>
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-ink-600/40">
                <span className="text-xs text-ink-400 font-mono">{post.date}</span>
                <span className="text-xs text-green-400 font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                  Read →
                </span>
              </div>
            </Link>
